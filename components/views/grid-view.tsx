...
export function GridView() {
  const { grids, activeGridId, activeGrid, createGrid, deleteGrid, renameGrid, switchGrid, addCard, updateViewport } = useGrid()
  ...
  return (
    <div className="flex-1 flex min-h-0 overflow-hidden bg-[var(--bg)]">
      {/* Grid list sidebar */}
      <div className={`shrink-0 flex flex-col border-r border-[var(--border)] bg-[var(--bg-elevated)] transition-all duration-200 ${sidebarCollapsed ? 'w-0 overflow-hidden' : 'w-52'}`}>
        <div className="flex items-center justify-between h-9 px-3 border-b border-[var(--border)]">
          <span className="text-[11px] font-semibold text-[var(--text-secondary)] uppercase tracking-wider">Grids</span>
          <button
            onClick={() => createGrid()}
